...
function _bookmarkletCode(){
  // The bookmarklet runs on the listing page, grabs HTML + URL, POSTs to parse endpoint,
  // then auto-saves by opening lockboxiq.com/?autosave= in a new tab
  return `javascript:void((function(){var u=location.href;var h=['zillow.com','realtor.com','redfin.com'];if(!h.some(function(d){return u.includes(d)})){alert('Open a Zillow, Realtor, or Redfin listing first');return;}var html=document.documentElement.outerHTML;var overlay=document.createElement('div');overlay.style.cssText='position:fixed;top:0;left:0;right:0;z-index:999999;background:linear-gradient(135deg,rgb(13,148,136),rgb(20,184,166));color:white;padding:12px 20px;font:600 14px/1.4 system-ui;text-align:center;box-shadow:0 2px 12px rgba(0,0,0,.3)';overlay.textContent='📌 Sending to LockboxIQ...';document.body.appendChild(overlay);fetch('https://tgborqvdkujajsggfbcy.supabase.co/functions/v1/fetch-listing',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({html:html.slice(0,200000),source_url:u})}).then(function(r){return r.json()}).then(function(d){if(d.error){overlay.style.background='rgb(220,38,38)';overlay.textContent='\\u274c '+d.error;setTimeout(function(){overlay.remove()},4000);return;}var p=[];if(d.price)p.push('$'+Number(d.price).toLocaleString());if(d.beds)p.push(d.beds+'bd');if(d.baths)p.push(d.baths+'ba');if(d.sqft)p.push(d.sqft.toLocaleString()+' sqft');overlay.innerHTML='\\u2705 <strong>'+p.join(' / ')+'</strong> \\u2014 saving to LockboxIQ...';window.open('https://lockboxiq.com/?autosave='+encodeURIComponent(JSON.stringify(d)),'_blank');overlay.innerHTML='\\u2705 <strong>'+p.join(' / ')+'</strong> \\u2014 Saved! <a href=https://lockboxiq.com/ target=_blank style=color:white;text-decoration:underline>Open Dashboard</a>';setTimeout(function(){overlay.remove()},6000);}).catch(function(e){overlay.style.background='rgb(220,38,38)';overlay.textContent='Failed: '+e.message;setTimeout(function(){overlay.remove()},4000);});})())`;
}
